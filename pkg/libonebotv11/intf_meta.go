...
type HeartbeatMetaEvent struct {
	MetaEvent
	Status   map[string]interface{} `json:"status"`   // OneBot 状态, 与 get_status 动作响应数据一致
	Interval int64                  `json:"interval"` // 到下次心跳的间隔，单位: 毫秒
}

// MakeHeartbeatMetaEvent 构造一个心跳元事件.
func MakeHeartbeatMetaEvent(interval int64) HeartbeatMetaEvent {
	return HeartbeatMetaEvent{
		MetaEvent: MakeMetaEvent("heartbeat"),
		Interval:  interval,
	}
}
